      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // ie. px = 0, py = 0, and psi = 0 degrees
          VectorXd car_x(N);
          VectorXd car_y(N);
          for(int i = 0; i < N; i++) {
            const double dx = ptsx[i] - px;
            const double dy = ptsy[i] - py;
            car_x[i] = dx * cos_psi - dy * sin_psi;
            car_y[i] = dy * cos_psi + dx * sin_psi;
          }

          auto coeffs = polyfit(car_x, car_y, 3); // Fit N waypoints with 3rd order polynomial
          const double cte = coeffs[0];
          const double epsi = -atan(coeffs[1]);

          //****Kinematic Vehicle Model****//
          // this model predicts the state of the vehicle at the
          // time that the control will be implemented
          // ie. (current time "t" + delay "dt")
          ...
          double steer_value = results[0]/ deg2rad(25); // convert back to range of [-1,1]
